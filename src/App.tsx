...
export default function App() {
  ...
  const [isAdmin, setIsAdmin] = useState(() => {
    return sessionStorage.getItem('partnerpro_is_admin') === 'true';
  });
  ...
  const handleVerifyPassword = (e: React.FormEvent) => {
    e.preventDefault();
    if (passwordInput === '9402') {
      setIsAdmin(true);
      sessionStorage.setItem('partnerpro_is_admin', 'true');
      ...
      if (pendingAction) {
        if (pendingAction.type === 'add') {
          openAddForm();
        } else if (pendingAction.type === 'edit' && pendingAction.vendor) {
          openEditForm(pendingAction.vendor);
        } else if (pendingAction.type === 'delete' && pendingAction.vendor) {
          initiateDelete(pendingAction.vendor, pendingAction.e as any);
        }
        setPendingAction(null);
      }
    } else {
      setPasswordError('密碼錯誤，請重新輸入！(提示：合約解鎖密碼)');
      setPasswordInput('');
    }
  };
  ...
  const handleAdminToggle = () => {
    if (isAdmin) {
      setIsAdmin(false);
      sessionStorage.removeItem('partnerpro_is_admin');
      showToast('已登出管理員模式', 'info');
    } else {
  ...
  const handleEditVendorClick = (vendor: Vendor, e?: React.MouseEvent) => {
    ...
    if (!isAdmin) {
      setPendingAction({ type: 'edit', vendor });
      setIsPasswordModalOpen(true);
      setPasswordInput('');
      setPasswordError('');
    } else {
      openEditForm(vendor, e);
    }
  };

  const handleDeleteVendorClick = (vendor: Vendor, e: React.MouseEvent) => {
    if (e) e.stopPropagation();
    if (!isAdmin) {
      setPendingAction({ type: 'delete', vendor, e });
      setIsPasswordModalOpen(true);
      setPasswordInput('');
      setPasswordError('');
    } else {
      initiateDelete(vendor, e);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#fafbfc] text-slate-800 antialiased font-sans pb-16 selection:bg-indigo-100 selection:text-indigo-950">
      ...
      <header className="bg-white border-b border-slate-200 h-20 flex items-center justify-between px-4 sm:px-8 shrink-0 sticky top-0 z-30 shadow-xs backdrop-blur-md bg-white/95">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            title={isAdmin ? "點擊登出管理員模式" : "點擊登入管理員模式"}
          ...
          >
            {isAdmin ? (
              <>
                ...
                <span className="hidden sm:inline">管理員 (已解鎖)</span>
                <span className="sm:hidden">管理員</span>
              </>
            ) : (
              <>
                  ...
                  <div className="p-8 border-b border-slate-100 flex-1 overflow-y-auto space-y-6">
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <div className="flex gap-2">
                        <button
                          onClick={(e) => handleEditVendorClick(selectedVendor, e)}
                        ...
                        <button
                          onClick={(e) => handleDeleteVendorClick(selectedVendor, e)}
      ...
      <AnimatePresence>
              ...
              >
                ...
                <div className="bg-slate-50 px-6 py-4 border-t border-slate-100 flex items-center justify-between">
                  <button
                    onClick={(e) => {
                      setSelectedVendor(null);
                      handleDeleteVendorClick(selectedVendor, e);
                    }}
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={(e) => {
                        setSelectedVendor(null);
                        handleEditVendorClick(selectedVendor, e);
                      }}
        ...
        {/* =========================================
            4. ADMIN PASSWORD VALIDATION MODAL
            ========================================= */}
        {isPasswordModalOpen && (
          <>
            {/* Backdrop */}
            ...
            >
              ...
              <motion.div
                ...
                transition={{ type: 'spring', duration: 0.4 }}
                onClick={(e) => e.stopPropagation()}
                className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden border border-slate-100 z-50"
              >
                <div className="p-6">
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      <h3 className="text-base font-bold text-slate-900">管理員功能解鎖</h3>
                      <p className="text-xs text-slate-500 mt-0.5">請輸入 4 位數解鎖密碼以進行管理</p>
                    </div>
                  </div>
                  ...
                  <form onSubmit={handleVerifyPassword} className="space-y-4">
                    <div>
                      ...
                      <input
                        ...
                        placeholder="請輸入密碼"
                        autoFocus
                        className={`w-full px-3.5 py-2.5 text-sm bg-slate-50 border rounded-xl outline-none focus:bg-white focus:ring-2 focus:ring-indigo-500 transition text-center font-mono tracking-widest text-lg ${
                          passwordError ? 'border-rose-300 ring-2 ring-rose-50' : 'border-slate-200'
                        }`}
                      />
                      {passwordError && (
                    ...
                    <div className="flex items-center justify-end gap-2 pt-2">
                      ...
                      <button
                        type="submit"
                        className="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold rounded-lg shadow transition active:scale-95"
                      >
                        確認解鎖
                      </button>
                    </div>
